...
def plot_certain_val_domain(ax, paths_to_csv, val=6, domain="x", mode="bar"):
    assert len(paths_to_csv) == 2
    assert "val" in paths_to_csv[0]
    assert "ood" in paths_to_csv[1]

    if domain == "x":
        df_val = pd.read_csv(paths_to_csv[0], index_col=0, usecols=[0] + list(range(1, 8)))
        df_all = pd.read_csv(paths_to_csv[1], index_col=0, usecols=[0] + list(range(1, 8)))
        x = np.arange(0, 7)
    elif domain == "z":
        df_val = pd.read_csv(paths_to_csv[0], index_col=0, usecols=[0] + list(range(8, 15)))
        df_all = pd.read_csv(paths_to_csv[1], index_col=0, usecols=[0] + list(range(8, 15)))
        x = np.arange(7, 14)
    ...
    if mode == "line":
        try:
            df_val_series = get_rows(df_val, "symm0.2") * 100
            df_all_series = get_rows(df_all, "symm0.2") * 100
            plot_series_belt(ax, x, df_val_series, "w/ symmetry on val set", ["forestgreen", "forestgreen"], linestyle="--")
            plot_series_belt(ax, x, df_all_series, "w/ symmetry on ood set", ["forestgreen", "forestgreen"], linestyle="-")
        except Exception as e:
            print(f"Error processing symm {domain}: {e}")

        try:
            df_val_series = get_rows(df_val, "nosymm") * 100
            df_all_series = get_rows(df_all, "nosymm") * 100
            plot_series_belt(ax, x, df_val_series, "w/o symmetry on val set", ["sienna", "sienna"], linestyle="--")
            plot_series_belt(ax, x, df_all_series, "w/o symmetry on ood set", ["sienna", "sienna"], linestyle="-")
        except Exception as e:
            print(f"Error processing nosymm {domain}: {e}")

    elif mode == "bar":
        symm_x = get_rows(df_val, "symm0.2") * 100 if domain == "x" else None
        nosymm_x = get_rows(df_val, "nosymm") * 100 if domain == "x" else None
        symm_z = get_rows(df_val, "symm0.2") * 100 if domain == "z" else None
        nosymm_z = get_rows(df_val, "nosymm") * 100 if domain == "z" else None

        data_groups = [
            ("w/ symm on X", symm_x, "forestgreen"),
            ("w/ symm on Z", symm_z, "mediumseagreen"),
            ("w/o symm on X", nosymm_x, "sienna"),
            ("w/o symm on Z", nosymm_z, "peru"),
        ]
        bar_width = 0.2
        offsets = np.linspace(-1.5, 1.5, len(data_groups)) * bar_width

        for offset, (label, data, color) in zip(offsets, data_groups):
            if data is not None and not data.empty:
                mean_vals = data.mean(axis=0).values
                std_vals = data.std(axis=0).values
...
def plot_main_exp():
    ...
    # plt.savefig("experiments/inducement/results/figures/performance_plot.svg", transparent=True, dpi=500)

    plt.savefig("experiments/transition/results/archive/figures/transition_performance_plot_1105.svg", dpi=500, transparent=True)
...
def plot_downstream_exp():
    ...
    plot_certain_val_domain(
        ax=axs[0, 1],
        paths_to_csv=[
            "experiments/inducement/results/csv/major_sax_val6_induced_minordown_val_0906.csv",
            "experiments/inducement/results/csv/major_sax_val6_induced_minordown_all_0906.csv",
        ],
        val=6,
        domain="x",
    )
    ...
    plot_certain_val_domain(
        ax=axs[0, 0],
        paths_to_csv=[
            "experiments/inducement/results/csv/major_sax_val3_induced_minordown_val_0906.csv",
            "experiments/inducement/results/csv/major_sax_val3_induced_minordown_all_0906.csv",
        ],
        val=3,
        domain="x",
    )
    ...
    plt.savefig("experiments/inducement/results/figures/performance_downstream_plot.pdf")
...
def plot_interval_probing_exp():
    ...
    csv_files = [
        "experiments/transition/results/archive/csv/major_sax_val3_transition_probe_1102.csv",
        "experiments/transition/results/archive/csv/major_sax_val6_transition_probe_1102.csv",
    ]
    bar_labels = [
        "w/ symmetry",
        # "w/ symmetry (K=1)",
    ...
    plt.savefig("experiments/transition/results/archive/figures/transition_interval_probing_plot.svg", transparent=True, dpi=500)
