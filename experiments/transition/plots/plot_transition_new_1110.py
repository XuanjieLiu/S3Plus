...
def plot_certain_val(ax, paths_to_csv, val=6):
    ...
    # symm_x_val_together = symm_x_val.mean(axis=1).to_frame()
    # nosymm_x_val_together = nosymm_x_val.mean(axis=1).to_frame()
    # symm_z_val_together = symm_z_val.mean(axis=1).to_frame()
    # nosymm_z_val_together = nosymm_z_val.mean(axis=1).to_frame()

    # symm_x_all_together = symm_x_all.mean(axis=1).to_frame()
    # nosymm_x_all_together = nosymm_x_all.mean(axis=1).to_frame()
    # symm_z_all_together = symm_z_all.mean(axis=1).to_frame()
    # nosymm_z_all_together = nosymm_z_all.mean(axis=1).to_frame()

    symm_x_val_together = symm_x_val.iloc[:, :5].mean(axis=1).to_frame()
    nosymm_x_val_together = nosymm_x_val.iloc[:, :5].mean(axis=1).to_frame()
    symm_z_val_together = symm_z_val.iloc[:, :5].mean(axis=1).to_frame()
    nosymm_z_val_together = nosymm_z_val.iloc[:, :5].mean(axis=1).to_frame()

    symm_x_all_together = symm_x_all.iloc[:, :5].mean(axis=1).to_frame()
    nosymm_x_all_together = nosymm_x_all.iloc[:, :5].mean(axis=1).to_frame()
    symm_z_all_together = symm_z_all.iloc[:, :5].mean(axis=1).to_frame()
    nosymm_z_all_together = nosymm_z_all.iloc[:, :5].mean(axis=1).to_frame()
    ...
    ax.set_ylabel("Prediction Accuracy in Next 5 Steps (%)")
    ax.set_ylim(50, 100)
...
def plot_main_exp():
    ...
    plt.savefig("experiments/transition/results/archive/figures/transition_performance_plot_1110.png", dpi=500, transparent=True)
