...
class PlusTrainer:
    ...
    def train(self):
        ...
        for epoch_num in range(start_epoch, self.max_iter_num):
            ...
            for batch_ndx, sample in enumerate(self.loader):
                ...
                plus_loss = self.bi_plus_loss(za, zb, zc, data[2], labels)
    ...
    def bi_plus_loss(self, za, zb, zc, imgs_c, labels):
        plus_loss1 = self.plus_loss(za, zb, zc, imgs_c, labels)
        plus_loss2 = self.plus_loss(zb, za, zc, imgs_c, labels)
        plus_loss = (
    ...
    def plus_loss(self, za, zb, zc, imgs_c, labels):
        z_s_list = []
        label_a = [parse_label(x) for x in labels[0]]
        for i in range(0, len(label_a)):
            if label_a[i] == 0:
                z_s_i = zb[i][self.latent_code_1:]
            else:
                z_s_i = za[i][..., self.latent_code_1:] if random.random() > 0.5 else zb[i][..., self.latent_code_1:]
