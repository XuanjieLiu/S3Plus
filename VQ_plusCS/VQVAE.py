...
class Encoder(nn.Module):
    ...
    def __init__(self, latent_code_num):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(IMG_CHANNEL, FIRST_CH_NUM, kernel_size=(4, 4), stride=(2, 2), padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(FIRST_CH_NUM, FIRST_CH_NUM, kernel_size=(4, 4), stride=(2, 2), padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(FIRST_CH_NUM, FIRST_CH_NUM * 2, kernel_size=(4, 4), stride=(2, 2), padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(FIRST_CH_NUM * 2, LAST_CN_NUM, kernel_size=(4, 4), stride=(2, 2), padding=1),
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_code_num):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(LAST_CN_NUM, FIRST_CH_NUM * 2, kernel_size=(4, 4), stride=(2, 2), padding=1),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(FIRST_CH_NUM * 2, FIRST_CH_NUM, kernel_size=(4, 4), stride=(2, 2), padding=1),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(FIRST_CH_NUM, FIRST_CH_NUM, kernel_size=(4, 4), stride=(2, 2), padding=1),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(FIRST_CH_NUM, IMG_CHANNEL, kernel_size=(4, 4), stride=(2, 2), padding=1),
        )
...
class VQVAE(nn.Module):
    ...
    def __init__(self, config):
        ...
        vq_code_num = self.latent_code_1 + self.latent_code_2 if self.isVQStyle else self.latent_code_1
        self.encoder = Encoder(self.latent_code_num)
        self.vq_layer = VectorQuantizer(vq_code_num, self.embeddings_num, self.commitment_scalar)
        self.decoder = Decoder(self.latent_code_num)
        self.mse_loss = nn.MSELoss(reduction='mean')
        self.VQafterPlus = config['isVQPlus'] and not config['isVQStyle']
        assert not (config['isVQPlus'] and config['isVQStyle']), "isVQPlus and isVQPlus cannot both be true"
    ...
    def plus(self, z_a, z_b):
        ...
        if not self.VQafterPlus:
            return z_plus, torch.zeros(1)[0]
        else:
            e_plus, e_q_loss = self.vq_layer(z_plus)
            return e_plus, e_q_loss
    ...
    def batch_encode_to_z(self, x):
        ...
        if self.isVQStyle:
            ...
            return torch.cat((e_c, z_s), -1), e_q_loss
